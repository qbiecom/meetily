    ...
    match config.provider.as_str() {
        ...
        "sherpaOnnx" => {
            ...
            if let Err(init_error) = crate::sherpa_engine::commands::sherpa_init().await {
                warn!("❌ Failed to initialize Sherpa ONNX engine: {}", init_error);
                return Err(format!(
                    "Failed to initialize Sherpa ONNX speech recognition: {}",
                    init_error
                ));
            }
            ...
            match crate::sherpa_engine::commands::sherpa_validate_model_ready_with_config(app).await
            {
                Ok(model_name) => {
                    info!(
                        "✅ Sherpa ONNX model validation successful: {} is ready",
                        model_name
                    );
                    Ok(())
                }
                Err(e) => {
                    warn!("❌ Sherpa ONNX model validation failed: {}", e);
                    Err(e)
                }
            }
        }
        other => {
            ...
            Err(format!(
                "Provider '{}' is not supported for local transcription. Please select 'localWhisper', 'parakeet', or 'sherpaOnnx'.",
                other
            ))
        }
    }
...
pub async fn get_or_init_transcription_engine<R: Runtime>(
    ...
    match config.provider.as_str() {
        ...
        "sherpaOnnx" => {
            ...
            match engine {
                Some(engine) if engine.is_model_loaded().await => Ok(TranscriptionEngine::Provider(Arc::new(
                    crate::audio::transcription::SherpaProvider::new(engine),
                ))),
                Some(_) => Err("Sherpa ONNX engine initialized but no model loaded. This should not happen after validation.".to_string()),
                None => Err("Sherpa ONNX engine not initialized. This should not happen after validation.".to_string()),
            }
        }
        "localWhisper" | _ => {
