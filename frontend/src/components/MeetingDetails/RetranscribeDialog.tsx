...
export function RetranscribeDialog({
  ...
  const isAutoLanguageOnlyModel = selectedModelDetails?.provider === 'parakeet' || selectedModelDetails?.provider === 'sherpaOnnx';
  ...
  const handleStartRetranscription = async () => {
    ...
    try {
      const languageToSend = isAutoLanguageOnlyModel ? null : selectedLang === 'auto' ? null : selectedLang;
      await Analytics.track('enhance_transcript_started', {
        language: isAutoLanguageOnlyModel ? 'auto' : (selectedLang === 'auto' ? 'auto' : selectedLang),
