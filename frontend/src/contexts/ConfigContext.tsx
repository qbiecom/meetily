...
export function ConfigProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const loadAllApiKeys = async () => {
      try {
        const providers = ['claude', 'groq', 'openai', 'openrouter', 'kilo-gateway'];
  ...
  const modelOptions: Record<ModelConfig['provider'], string[]> = {
    ...
    openrouter: [],
    'kilo-gateway': ['kilo/auto', 'anthropic/claude-sonnet-4.6', 'openai/gpt-5.4', 'google/gemini-3.1-pro-preview', 'openrouter/free'],
